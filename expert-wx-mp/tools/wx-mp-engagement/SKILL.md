...
**失效重登流程**（走本工具自己）：
```bash
wx-mp-engagement login           # camoufox 无头截 QR PNG 落 workspace 可发送路径
...
- **二维码产物**：`login` 默认写入 `~/.openclaw/workspace-main/wx_mp/calibration/auth/qr-wx-mp.png`（可用 `WX_MP_QR_FILE` 覆盖）；返回成功前验证文件存在、非空、PNG CRC 与压缩图像数据可解码。camoufox JSON 即使进程退出码为 0，只要 `success:false` 也必须报 `ok:false`，禁止假成功。
