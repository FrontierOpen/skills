...
**登记表是 client 侧 id → 本地 CSS 路径映射，不持久化主题内容。** CSS 内容随发布请求一起发送，发布完成后不留存于中转层。
...
> 注：发布统一走 `publish_wx_mp.py`，当 theme 参数指向本地 `.css` 文件路径、或为 `wx_mp/wenyan-theme/index.json` 登记的自定义 id 时，脚本会直接读取 CSS 内容并随 direct 请求提交；不会在客户端之外持久化保存主题。
