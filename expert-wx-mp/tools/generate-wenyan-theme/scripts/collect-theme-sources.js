...
function parseArgs() {
  ...
  return {
    ...
    wxHunter: "wx-mp-hunter",
  };
}

async function assertCommandAvailable(command, label) {
  const pathEntries = String(process.env.PATH ?? "").split(delimiter);
  for (const entry of pathEntries) {
    const candidate = resolve(entry || ".", command);
    const info = await stat(candidate).catch(() => null);
    if (!info || !info.isFile()) continue;
    const executable = await access(candidate, constants.X_OK).then(() => true).catch(() => false);
    if (executable) return;
  }
  fail(`在 PATH 中找不到可执行的 ${label}: ${command}`);
}
...
async function main() {
  ...
  await assertCommandAvailable(options.wxHunter, "wx-mp-hunter wrapper");
