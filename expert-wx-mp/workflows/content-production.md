...
| 署名 | 用户指定或有要求 `author` 时逐字使用；没有则留空，不编造 |
...
- 指定 `author`、`source_url` 时逐字或按原意执行；未指定时 author 和 source_url 都不填。
...
1. 用户指定主题 -> 使用指定主题。
2. 用户未指定 -> Agent 根据文章内容自主选择：先看 `wenyan-theme/index.json` 有无可复用自定义主题，再按 `wx-mp-publisher` 的决策树从内置主题中挑选，不回退到默认中转层。
3. 用户要求对标排版（“照着这篇的排版来”）-> 调 `generate-wenyan-theme` 提取参考文章排版风格生成主题，登记到 `wenyan-theme/index.json`。
4. 自定义主题把登记的 `theme-id` 直接传给 `wx-mp-publisher`（脚本经 `wenyan-theme/index.json` 解析出 CSS 并随 direct 请求提交）；直接传 `.css` 文件路径也可以。
