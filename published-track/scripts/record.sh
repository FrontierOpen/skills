...
# --help/-h is a usage probe; honor it before touching the DB.
for arg in "$@"; do
  if [ "$arg" = "--help" ] || [ "$arg" = "-h" ]; then
    cat <<'EOF'
Usage: published-track record --platform <name> --title <title> \
  --content-type <article|video|post> --source-folder <folder> [options]

Record a published item in pub_<platform>. Repeating the same source folder
on the same publish date updates the existing row instead of inserting one.

Required:
  --platform <name>          Platform table suffix (for example wx_mp, xhs).
  --title <title>            Published title (full description for wx_channel).
  --content-type <type>      article, video, or post.
  --source-folder <folder>   Work folder; used to discover dna-meta.json.

Optional:
  --publish-url <url>        Public URL (may be empty for failed publishes).
  --publish-date <YYYY-MM-DD> Date; defaults to today.
  --distribute-status <0|1|2> Pending, no distribution, or distributed.
  --dna-id <id>              Override dna-meta.json DNA association.
  --account <alias>          Publishing account alias.
  --notes <text>             Additional notes.

Output: JSON on stdout. action is inserted or updated on success.
EOF
    exit 0
  fi
done
